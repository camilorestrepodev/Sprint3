...
export const submitForm = async (form, url, editForm) => {
    form.addEventListener("submit", async (e) => {
        ...
        valuesForm.forEach((valueInput) => {
            if (valueInput.id) {
                infoInmueble[valueInput.id] = valueInput.value;
            }
        });
        ...
        const noHayCamposVacios = validacionCamposForm(arrayLabels, infoInmueble);
        //Acción para editar
        if (noHayCamposVacios && editForm) {
            await putDataFetch(url, infoInmueble);
            Swal.fire(
                "¡Excelente!",
                "Se ha actualizado exitosamente el Inmueble",
                "success"
            ).then(() => {
        ...
        if (noHayCamposVacios && !editForm) {
            await postDataFetch(url, infoInmueble);
            Swal.fire(
                "¡Excelente!",
                "Se ha creado exitosamente el inmueble",
                "success"
            ).then(() => {
...
const validacionCamposForm = (arrayLabels, inmuebleInfo) => {
    ...
    let keyStr = "";
    for (const key in inmuebleInfo) {
        const propertyInmueble = inmuebleInfo[key];
